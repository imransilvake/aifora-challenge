# Aifora Test
Aifora test to implement a reusable “Workflow” component that is flexible and adaptable.

<p align="center">
	<img src="preview.png" width="100%">
</p>


## Content
Implement a reusable “Workflow” component that is flexible and adaptable. You should consider the following:
- [X] This component can be added to any page.
- [X] This component should have a clear Input/Output interface.
- [X] The selected bucket should be highligest as shown in the screenshot.
- [X] The number of buckets can change in the future.
- [X] The checkboxes can be different for each bucket.
- [X] There may be more buttons to place in the future.
